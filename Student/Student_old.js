...
function signIn() {
    setTimeout(async () => {
        ...
        const userPassword = document.querySelector("#userPass").value
        if (userEmail == '') {
            ...
            if (!document.querySelector('#parentBox').checked) {
                ...
                .eq('email', userEmail)
                if (data.length > 0) {
                    alert('There is already an account with this email, please log in')
                } else {
                ...
                const {dataTwo, error: errorTwo } = await supabase.auth.signUp({
                    email: userEmail,
                    password: userPassword,
                })
                errorTwo ? console.log("Error is: ", errorTwo) : console.log("nice", dataTwo)
            }
        }
    }, 1);
}
